...
class TestComputer(unittest.TestCase):
    ...
    def tests_from_09(self):
        ...
        program.execute([])
        self.assertEqual(program.outputs, [109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99])

        program = computer.Computer([1102,34915192,34915192,7,4,7,99,0])
        program.execute([])
        self.assertEqual(len(str(program.outputs[0])), 16)

        program = computer.Computer([104,1125899906842624,99])
        program.execute([])
        self.assertEqual(program.outputs, [1125899906842624])
        ...
        program.execute([1])
        self.assertEqual(program.outputs, [3765554916])
